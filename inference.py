...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            joinFactorTup = joinFactorsByVariable(factorList, variable)
            factorList = joinFactorTup[0]
            if len(joinFactorTup[1].unconditionedVariables()) > 1:
                factorList.append(eliminate(joinFactorTup[1], variable))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None:
            ...
            obsProb = busters.getObservationProbability(noisyDistance, trueDist)
            return obsProb
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            prop = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())
            self.beliefs[position] = prop * self.beliefs[position]
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newProp = DiscreteDistribution()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            prop = self.beliefs[oldPos]
            for newPos in newPosDist.keys():
                newProp[newPos] += prop * newPosDist[newPos]
        self.beliefs = newProp
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles += self.legalPositions[0:(self.numParticles % len(self.legalPositions))]
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        believe = DiscreteDistribution()
        for particle in self.particles:
            if particle in believe:
                believe[particle] += 1
            else:
                believe[particle] = 1
        believe.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            if oldPos in storage:
                """Reduce getPositionDistribution call by store calculated newPostDist in a dict"""
                self.particles[i] = storage[oldPos].sample()
            else:
                newPosDist = self.getPositionDistribution(gameState, oldPos)
                storage[oldPos] = newPosDist
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        random.shuffle(perm)
        for i in range(self.numParticles // len(perm)):
            self.particles += perm
        self.particles += perm[:(self.numParticles % len(perm))]
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                prop *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
            distribution[particle] += prop 
        if distribution.total() != 0:
            distribution.normalize()
            self.beliefs = distribution     
            for i in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                if not (oldParticle, i) in storage:
                    newPosDist = self.getPositionDistribution(gameState, prevGhostPositions, i, self.ghostAgents[i])
                    storage[(oldParticle, i)] = newPosDist
                    newParticle[i] = newPosDist.sample()
                else:
                    newParticle[i] = storage[(oldParticle, i)].sample()
            """*** END YOUR CODE HERE ***"""
