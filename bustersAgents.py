...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for ghostDist in livingGhostPositionDistributions:
            curDist = self.distancer.getDistance(pacmanPosition, ghostDist.argMax())
            if curDist < minDist:
                minDist = curDist
                closetGhostPos = ghostDist.argMax()
        minDist = 100000
        ans = 0
        for action in legal:
            newDist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closetGhostPos)
            if newDist < minDist:
                minDist, ans = newDist, action
        return ans
        "*** END YOUR CODE HERE ***"
